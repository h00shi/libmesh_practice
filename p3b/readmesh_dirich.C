...
Real exact_solution (const Real x, const Real y, const Real z = 0.){
	...
	const double tet = atan(y/x);
	const double cs2 = cos(2*tet);

	//return sin(4/3. * M_PI * (r - 0.25));
	//return sin(4/3. * M_PI * (r - 0.25)) * cs2;
	return sin(4/3. * M_PI * (r - 0.25)) * exp(cs2);
}
Real right_hand_side(const Real x, const Real y, const Real z = 0.){
	...
	const double tet = atan(y/x);
	const double cs2 = cos(2*tet);
	const double sn2 = sin(2*tet);

	//const double urr =
	//		4/3. * M_PI * sin(M_PI/6. * (5-8*r)) / r-
	//		16/9. * M_PI * M_PI * cos(M_PI/6. * (5-8*r));
	//return -urr;

	//const double urr =
	//		4/3. * M_PI * sin(M_PI/6. * (5-8*r)) / r-
	//		16/9. * M_PI * M_PI * cos(M_PI/6. * (5-8*r));
	//return cs2 * (-urr + 4/r2 * sin(4/3. * M_PI * (r - 0.25)) );

	const double urr =
			4/3. * M_PI * sin(M_PI/6. * (5-8*r)) / r-
			16/9. * M_PI * M_PI * cos(M_PI/6. * (5-8*r));
	return exp(cs2) * (-urr + 4/r2 * sin(4/3. * M_PI * (r - 0.25)) * (cs2 - sn2*sn2) );
}

void boundary_value (DenseVector<Number>& output,
                             const Point& p,
                             const Real)
{
	const double x = p(0) , y = p(1);

	if (b_exact_bdry) output(0)=exact_solution(x,y);
	else output(0) = 0;
}
                      ...
                      const std::string&) // unk_name, not needed
{
	...
	const double tet = atan(p(1)/p(0));
	const double cs2 = cos(2*tet);
	const double sn2 = sin(2*tet);


	//const double dudr = 4/3. * M_PI * cos(4/3. * M_PI *(r-0.25) );
	//const double dudt = 0;

	//const double dudr = 4/3. * M_PI * cos(4/3. * M_PI *(r-0.25) ) * cs2;
	//const double dudt = -2 * sn2 * sin(4/3. * M_PI * (r - 0.25)) / r;

	const double dudr = 4/3. * M_PI * cos(4/3. * M_PI *(r-0.25) ) * exp(cs2);
	const double dudt = sin(4/3. * M_PI *(r-0.25) ) * (-2*sn2*exp(cs2)) / r;
...
int main (int argc, char** argv)
{
	...
	b_sym_sol = getpot("sym_sol",false);
	...
	equation_systems.get_system("Poisson").add_variable("u", SECOND, HIERARCHIC);
	...
	boundary_ids.insert(401);
	std::vector<unsigned int> variables(1);
	variables[0] = 0;
	AnalyticFunction<> analytic_func_bdry(boundary_value);
		...
		const std::string& system_name)
{
	...
	for ( ; el != end_el ; ++el)
	{
		...
		for (unsigned int side=0; side<elem->n_sides(); side++)
			if ((elem->neighbor(side) == NULL) && (mesh.boundary_info->boundary_id(elem,side)==401))
			{
				...
				for (unsigned int qp=0; qp<qface.n_points(); qp++)
				{
					...
					DenseVector<Number> value; value.resize(1);
					boundary_value(value, qface_point[qp], 0);
					...
					for (unsigned int i=0; i<phi_face.size(); i++)
						Fe(i) += JxW_face[qp]*penalty*value(0)*phi_face[i][qp];
				}
			}
